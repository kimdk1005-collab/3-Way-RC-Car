...
char last_cmd = 's';    // 직전에 전송한 명령어 (디버깅용 유지)
/* USER CODE END PV */
...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  // DMA 방식을 사용하여 조이스틱 4개 채널 아날로그 값을 자동으로 버퍼에 수집 개시
  ...
  while (1)
  {
	  ...
	  if (joy1_y > 3000) {
		  ...
		  current_cmd = 'r'; // 우회전 (Right)
	  }
	  else if (joy2_x < 1000) {
		  current_cmd = 'l'; // 좌회전 (Left)
	  }
	  else {
		  current_cmd = 's'; // 입력 없음 -> 정지 (Stop)
	  }

	  // 3. 무조건 50ms마다 현재 상태를 계속 전송 (⭐ 블루투스 데이터 유실 방어벽 가동)
	  HAL_UART_Transmit(&huart1, (uint8_t*)&current_cmd, 1, 10);
	  last_cmd = current_cmd; // 디버깅용 변수 업데이트 유지

	  // 4. 무한 루프 과부하 방지 및 전송 주기 동기화 (초당 20회 송신)
	  HAL_Delay(50);
    /* USER CODE END WHILE */
